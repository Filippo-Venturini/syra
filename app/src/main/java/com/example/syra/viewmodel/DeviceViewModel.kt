...
class DeviceViewModel : ViewModel() {
    ...
    private val bedRoomDeviceList = listOf(Device("Shutter", TOPIC))
    private val livingRoomDeviceList = listOf<Device>() //listOf(Device("Living Room Device 1", TOPIC), Device("Living Room Device 2", TOPIC))
    private val kitchenDeviceList = listOf<Device>() //listOf(Device("Kitchen Device 1", TOPIC), Device("Kitchen Device 2", TOPIC))
    private val studyDeviceList = listOf<Device>()//listOf(Device("Study Device 1", TOPIC), Device("Study Device 2", TOPIC), Device("Study Device 3", TOPIC), Device("Study Device 4", TOPIC))
    private val allDevicesList = bedRoomDeviceList + livingRoomDeviceList + kitchenDeviceList + studyDeviceList
    ...
    fun switchAllOff(){
        allDevicesList.forEach{d ->
            mqttRepository.publishMessage(d.topic, SWITCH_0_OFF)
            mqttRepository.publishMessage(d.topic, SWITCH_1_OFF)
        }
    }
